...
import { Zap, Calendar, Download, CreditCard, TrendingUp, FileText, ChevronDown, Check, AlertCircle, User, MapPin, Gauge, Hash, Building2, X, PlusCircle } from 'lucide-react';
...
const ElectricityBillingDashboard = () => {
    ...
    receiptWindow.document.write(`
      ...
      <head>
        ...
        <style>
          ...
          .status-paid { background: #d1fae5; color: #065f46; padding: 5px 15px; border-radius: 20px; display: inline-block; }
      ...
      <body>
        ...
        <div class="section">
          ...
          <div class="detail-row">
            ...
            <span class="detail-value">${userData.consumerNumber}</span>
          </div>
          <div class="detail-row">
          ...
          <div class="detail-row">
            ...
            <span class="detail-value">${payment.billPeriod}</span>
          </div>
          <div class="detail-row">
            <span class="detail-label">Units Consumed:</span>
            <span class="detail-value">${payment.units} kWh</span>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <PageHeader
        ...
        description={
          <span className="text-violet-50 text-sm sm:text-base font-medium">
            {t('services.consumerNo')}: {userData.consumerNumber}
          </span>
        }
        icon={Zap}
        watermarkIcon={Zap}
        ...
        orb1Color="from-violet-400/15 to-purple-500/15"
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="lg:w-2/3 grid grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="bg-white rounded-xl shadow-md border-2 border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
                {/* Due Amount Card */}
                <div className="bg-gradient-to-br from-violet-600 to-indigo-700 rounded-3xl p-8 text-white shadow-xl transform transition-transform hover:scale-[1.02] flex flex-col justify-between min-h-[220px]">
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                      <div className="p-3 bg-white/20 backdrop-blur-md rounded-2xl">
                        <AlertCircle size={32} className="text-white" />
                      </div>
                      <h2 className="text-xl font-bold uppercase tracking-wider text-violet-100">{t('billing.currentBillDue')}</h2>
                    </div>
                    <div className="flex items-baseline gap-2 mt-2">
                      <span className="text-5xl font-extrabold tracking-tight">₹{currentBill.amount.toLocaleString()}</span>
                      <span className="text-xl font-medium text-violet-200">{t('billing.due')}</span>
                    </div>
                    <p className="mt-4 text-lg font-medium text-violet-100 flex items-center gap-2">
                      {t('billing.dueDate')}: <span className="bg-white/20 px-3 py-1 rounded-lg">{formatDate(currentBill.dueDate)}</span>
                    </p>
                  </div>
                  <button
                    onClick={openPaymentModal}
                    className="mt-6 w-full py-4 bg-white text-violet-700 rounded-2xl font-bold text-xl hover:bg-violet-50 transition-colors shadow-lg active:scale-95 flex items-center justify-center gap-3"
                  >
                    <CreditCard size={24} />
                    {t('billing.payNow')}
                  </button>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                  ...
                  {selectedBillPeriod && selectedBillPeriod !== 'custom' && (
                    <button
                      ...
                      className="px-6 py-3 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-colors bg-gradient-to-r from-violet-600 to-violet-700 text-white hover:from-violet-700 hover:to-violet-800 shadow-sm"
                    >
                      <Download size={18} />
          ...
          <div className="bg-white rounded-xl shadow-md border-2 border-gray-200 overflow-hidden">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-gray-200 bg-gray-50">
                      <th className="text-left py-5 px-5 text-sm font-bold text-gray-600 uppercase tracking-wide">{t('billing.dueDate')}</th>
                      <th className="text-left py-5 px-5 text-sm font-bold text-gray-600 uppercase tracking-wide">{t('billing.billingPeriod')}</th>
                      <th className="text-left py-5 px-5 text-sm font-bold text-gray-600 uppercase tracking-wide hidden sm:table-cell">{t('consumption.unitsConsumed')}</th>
                      <th className="text-left py-5 px-5 text-sm font-bold text-gray-600 uppercase tracking-wide">{t('billing.amount')}</th>
                      <th className="text-left py-5 px-5 text-sm font-bold text-gray-600 uppercase tracking-wide">{t('billing.status')}</th>
                      <th className="text-left py-5 px-5 text-sm font-bold text-gray-600 uppercase tracking-wide hidden md:table-cell">{t('billing.transactionId')}</th>
                  ...
                  <tbody>
                    {(showAllPayments ? paymentHistory : paymentHistory.slice(0, 3)).map((payment, index) => (
                      <tr key={payment.id} className={`border-b border-gray-100 hover:bg-violet-50/50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                        ...
                        <td className="py-5 px-5 text-base text-gray-700 hidden sm:table-cell">{payment.units} kWh</td>
                        <td className="py-5 px-5 text-base font-bold text-gray-900">₹{payment.amount.toLocaleString()}</td>
                        <td className="py-5 px-5">
                          <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-violet-100 text-violet-700 border border-violet-200 rounded-lg text-sm font-bold">
                            <Check size={16} />
                            {t('billing.paid')}
                          </span>
                        </td>
              ...
              {!showAllPayments && paymentHistory.length > 3 && (
                <div className="mt-4 text-center">
                  <button
                    ...
                    className="px-6 py-2.5 text-sm font-semibold text-violet-700 border-2 border-violet-200 hover:bg-violet-50 rounded-lg transition-colors"
                  >
            ...
            <div className="mx-5 mb-5 p-4 bg-amber-50 border-2 border-amber-200 rounded-lg flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <button
                  onClick={openPaymentModal}
                  className="mt-3 px-4 py-2 bg-gradient-to-r from-violet-600 to-violet-700 text-white rounded-lg font-bold text-xs hover:from-violet-700 hover:to-violet-800 transition-colors shadow-sm"
                >
                  {t('billing.payNow')}
                </button>
              </div>
...
export default ElectricityBillingDashboard;
