    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="w-full max-w-5xl my-4 rounded-3xl overflow-hidden shadow-2xl border-4 border-gray-200 bg-white transform transition-all scale-100">
        ...
        {paymentSuccess ? (
          ...
          <>
            {/* Official Header */}
            <div className="bg-gradient-to-r from-violet-700 to-violet-600 p-5 sm:p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h2 className="text-xl sm:text-2xl font-extrabold text-white tracking-wide">ELECTRICITY BILL PAYMENT</h2>
                    <p className="text-violet-100 text-base mt-0.5 font-medium">Consumer No: {userData?.consumerNumber}</p>
                  </div>
                </div>
                <button
                  onClick={handleClose}
            ...
            <div className="mx-6 mt-4">
              <div className="bg-white rounded-2xl border-2 border-gray-200 shadow-md overflow-hidden">
                ...
                <div className="p-5 flex items-center justify-between">
                  <div className="flex items-center gap-5">
                    <div className="w-16 h-16 bg-violet-50 rounded-2xl flex items-center justify-center border-2 border-violet-100">
                      <IndianRupee size={32} className="text-violet-600" />
                    </div>
                    <div>
                      <p className="text-xs text-gray-500 font-bold uppercase tracking-wide">Total Amount Due</p>
                      <p className="text-3xl font-extrabold text-gray-900 mt-0.5">₹{billData?.amount?.toLocaleString()}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="flex items-center justify-end gap-1.5 text-gray-500 mb-1">
                      <Clock size={16} />
                      <span className="text-xs font-bold uppercase tracking-wide">Billing Period</span>
                    </div>
                    <p className="text-xl font-bold text-gray-900">{billData?.billingPeriod}</p>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    {paymentMethods.map((method) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-4 rounded-xl border-2 text-center transition-all shadow-sm ${paymentMethod === method.id
                        ...
                        >
                          <IconComponent
                            size={24}
                            className={`mx-auto mb-2 ${paymentMethod === method.id ? 'text-violet-700' : 'text-gray-500'}`}
                          />
                          <span className={`font-bold text-sm block ${paymentMethod === method.id ? 'text-violet-800' : 'text-gray-700'}`}>
                            {method.name}
                          </span>
                        </button>
                      );
                  ...
                  <form onSubmit={handlePayment}>
                    {paymentMethod === 'upi' && (
                      <div className="text-center">
                        <div className="inline-block bg-gray-50 rounded-xl p-4 border border-gray-200">
                          <p className="text-xs font-semibold text-gray-700 mb-3 uppercase tracking-wide">Scan QR Code to Pay</p>
                          <div className="relative w-48 h-48 mx-auto">
                            <div className={`w-full h-full bg-white rounded-xl p-2 border-2 border-gray-300 shadow-inner transition-all duration-300 ${!showQR ? 'blur-md' : ''}`}>
                              <img
                                src={QrImage}
                                alt="Payment QR Code"
                            ...
                            {!showQR && (
                              <div className="absolute inset-0 flex items-center justify-center">
                                <button
                                  ...
                                  className="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg font-semibold text-xs shadow-lg transition-all flex items-center gap-2"
                                >
                                  <Eye size={14} />
                                  Show QR Code
                                </button>
                    ...
                    {paymentMethod === 'card' && (
                      <div className="space-y-4 max-w-xl mx-auto py-2">
                        <div>
                          <label className="block text-base font-bold text-gray-700 mb-1.5 ml-1">Card Number</label>
                          <input
                            type="text"
                            value={cardNumber}
                            onChange={(e) => setCardNumber(e.target.value.replace(/\D/g, '').slice(0, 16))}
                            placeholder="1234 5678 9012 3456"
                            className="w-full px-5 py-3 border-2 border-gray-300 rounded-xl text-lg tracking-wider focus:border-violet-600 focus:ring-4 focus:ring-violet-100 focus:outline-none transition-all bg-white"
                            required
                          />
                        </div>
                        <div>
                          <label className="block text-base font-bold text-gray-700 mb-1.5 ml-1">Cardholder Name</label>
                          <input
                            type="text"
                            value={cardName}
                            onChange={(e) => setCardName(e.target.value)}
                            placeholder="Name as on card"
                            className="w-full px-5 py-3 border-2 border-gray-300 rounded-xl text-lg focus:border-violet-600 focus:ring-4 focus:ring-violet-100 focus:outline-none transition-all bg-white"
                            required
                          />
                        </div>
                        <div className="grid grid-cols-2 gap-5">
                          <div>
                            <label className="block text-base font-bold text-gray-700 mb-1.5 ml-1">Expiry</label>
                            <input
                              type="text"
                              value={cardExpiry}
                              onChange={(e) => setCardExpiry(e.target.value)}
                              placeholder="MM/YY"
                              className="w-full px-5 py-3 border-2 border-gray-300 rounded-xl text-lg text-center focus:border-violet-600 focus:ring-4 focus:ring-violet-100 focus:outline-none transition-all bg-white"
                              required
                            />
                          </div>
                          <div>
                            <label className="block text-base font-bold text-gray-700 mb-1.5 ml-1">CVV</label>
                            <input
                              type="password"
                              value={cardCvv}
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          {['Paytm', 'PhonePe', 'Amazon Pay'].map((wallet) => (
                            <button
                              key={wallet}
                              type="button"
                              className="p-3 border-2 border-gray-200 rounded-lg hover:border-violet-400 hover:bg-violet-50 transition-all text-center"
                            >
                              <p className="font-semibold text-xs text-gray-800">{wallet}</p>
                            </button>
                          ))}
                        </div>
                      </div>
                    ...
                    {paymentMethod !== 'upi' && (
                      <div className="max-w-md mx-auto mt-6 mb-2">
                        <button
                          ...
                          className={`w-full px-6 py-4 rounded-xl font-bold text-lg text-white flex items-center justify-center gap-3 transition-all transform active:scale-95 ${isProcessing
